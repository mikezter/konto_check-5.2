...
int gettimeofday (struct timeval *tv, void *tz)
{
  ...
  if (frequency == -1) { /* use ftime; this yields only milliseconds resolution */
    ...
    QueryPerformanceCounter (&qpc);
    cnt = qpc.QuadPart;
    tv->tv_sec = sec = cnt / frequency + ct;
    cnt = (cnt - sec * frequency) * 1000000;
    tv->tv_usec = cnt / frequency;
  }
...
int main (int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++) {
    if (*argv[i] == '-')
      ...
      case 'G':
        ...
        for (j = 0, ptr1 = ptr; *ptr1 && j++ < 8;)if (!isdigit (*ptr1++)) {
            FORMAT_ERROR;
          }
        ...
        for (j = 0; *ptr1 && j++ < 8;)if (!isdigit (*ptr1++)) {
            FORMAT_ERROR;
          }
      ...
      case 'Z':
        ...
        switch (*ptr) {
        case '0':
          set_default_compression (COMPRESSION_NONE);
          break;

        case '1':
          set_default_compression (COMPRESSION_ZLIB);
          break;

        case '2':
          set_default_compression (COMPRESSION_BZIP2);
          break;

        case '3':
          set_default_compression (COMPRESSION_LZO);
          break;

        case '4':
          set_default_compression (COMPRESSION_LZMA);
          break;
  ...
  if (action == GEN_LUT) {
    ...
    if (ret < 0) {
      ...
      if (keine_iban_berechnung) {
        lut_keine_iban_berechnung (keine_iban_berechnung, outputname, set);
      }
      ...
      *buffer = 0;
      ...
      if (*buffer) {
          ...
          *ptr = 0; /* \r bzw. \n am Zeilenende entfernen (stört i.A.) */
          ...
          case 'b':   /* Bankinfos anzeigen (holen über lut_multiple()) */
            if (* (buffer + 2) == 'i') { /* Dateien per Index holen */
              ...
              while (isdigit (*ptr)) {
                ptr++;  /* erste Zahl überspringen (Index der Bank) */
              }
        ...
        * (buffer + 999) = 0; /* nur zur Sicherheit Stringende eintragen */
        ...
        for (ptr = buffer, dptr = ptr1 = buffer + 1000, i = 0; isalnum (*ptr) && i < 8;) {
          *dptr++ = *ptr++; /* Bankleitzahl kopieren */
        }

        *dptr++ = 0;

        while (*ptr && !isdigit (*ptr)) {
          ptr++;  /* Anfang der Kontonummer suchen */
        }

        for (ptr2 = dptr, i = 0; isdigit (*ptr) && i < 10;) {
          *dptr++ = *ptr++; /* Kontonummer kopieren */
        }
  ...
  if (!inputname) {
    inputname = "blz_in.txt";
  }

  if (!outputname) {
    outputname = "blz_out.txt";
  }
  ...
  if (verbose) {
    fprintf (stderr, "%d Byte gelesen aus %s\n", cnt, inputname);
  }
  ...
  if (action == FAST)for (eptr = buffer + cnt, sptr = buffer, dptr = dbuffer, out_cnt = zeilen = konten = 0; sptr < eptr;) {
      ...
      if (dptr > dbuffer + OUTPUT_BUFSIZE - 2000) {
        ...
        if (cnt != (dptr - dbuffer)) {
          perror ("Schreibfehler bei Ausgabe");
        }
  ...
  /******************** langsamer Modus ********************/
      ...
      while (*ptr1 == ' ' || *ptr1 == '\t') {
        ptr1++;  /* Blanks am Zeilenanfang überspringen */
      }
